...
public class EasyTracker {
    ...
    public static EasyTracker getTracker() {
        if (EasyTracker.instance == null) {
            EasyTracker.instance = new EasyTracker();
        }
        return EasyTracker.instance;
    }
    ...
    private void initializeTracker() {
        this.queueToTrackerThreadIfEnabled(new Runnable() {
            @Override
            public void run() {
     ...
     */
    void setTrackerDelegate(final GoogleAnalyticsTrackerDelegate d) {
        if (this.gaEnabled) {
            this.tracker = d;
            this.initializeTracker();
        }
    ...
    private void loadParameters() {
        ...
        if (this.gaAccountId != null) {
            ...
            this.autoActivityTracking =
                    this.parameterFetcher.getBoolean("ga_auto_activity_tracking");
    ...
    public void setContext(final Context ctx) {
        if (ctx == null) {
            Log.e(EasyTracker.LOG_TAG, "Context cannot be null");
        }
     ...
     */
    void setContext(final Context ctx, final ParameterLoader parameterLoader) {
        if (ctx == null) {
            Log.e(EasyTracker.LOG_TAG, "Context cannot be null");
        }
        if (this.gaContext == null) {
    ...
    public void trackActivityStart(final Activity activity) {
        ...
        final boolean startASession = this.sessionNeeded;
    ...
    public void trackActivityStop(final Activity activity) {
        ...
        final boolean sendEvent = this.sessionNeeded;
        this.queueToTrackerThreadIfEnabled(new Runnable() {
            @Override
            public void run() {
                if (sendEvent) {
    ...
    public void setReferrer(final String referrer) {
        this.queueToTrackerThreadIfEnabled(new Runnable() {
            ...
            @Override
            public void run() {
                EasyTracker.this.getGoogleAnalyticsTracker().setReferrer(referrer);
            }
    ...
    public void trackEvent(final String category, final String action, final String label,
        ...
        this.queueToTrackerThreadIfEnabled(new Runnable() {
            ...
            @Override
            public void run() {
                EasyTracker.this.getGoogleAnalyticsTracker().trackEvent(category, action, label, value);
            }
    ...
    public void setCustomVar(final int index, final String name, final String value) {
        this.queueToTrackerThreadIfEnabled(new Runnable() {
            @Override
            public void run() {
                EasyTracker.this.getGoogleAnalyticsTracker().setCustomVar(index, name, value);
            }
        });
    }
    ...
    public void setCustomVar(final int index, final String name, final String value, final int scope) {
        this.queueToTrackerThreadIfEnabled(new Runnable() {
            @Override
            public void run() {
                EasyTracker.this.getGoogleAnalyticsTracker().setCustomVar(index, name, value, scope);
            }
        });
    }
    ...
    public void trackPageView(final String name) {
        this.queueToTrackerThreadIfEnabled(new Runnable() {
            ...
            @Override
            public void run() {
                EasyTracker.this.getGoogleAnalyticsTracker().trackPageView(name);
            }
     ...
     */
    private void queueToTrackerThreadIfEnabled(final Runnable r) {
        if (this.gaEnabled) {
            synchronized (this.lock) {
                this.trackerQueue.add(r);
            }
        }
    }
    ...
    private class TrackerThread extends Thread {
        ...
        public void run() {
            while (true) {
                Runnable r;
                try {
                    r = EasyTracker.this.trackerQueue.take();
                    r.run();
