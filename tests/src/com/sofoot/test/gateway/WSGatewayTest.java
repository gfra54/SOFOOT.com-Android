...
public class WSGatewayTest extends AndroidTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        this.connectivityManager = (ConnectivityManager) this.getContext().getSystemService(
                Context.CONNECTIVITY_SERVICE);

        this.gateway = new WSGateway(this.connectivityManager, this.getContext().getString(R.string.app_name) + "/"
                + this.getContext().getString(R.string.app_version), new HttpHost("www.sofoot.com", 80));
    ...
    public void testGetRequestAcceptGzip() {
        final Header[] headers = this.gateway.buildGetRequest("/ws.php", new ArrayList<BasicNameValuePair>())
                .getHeaders("Accept-Encoding");
        Assert.assertTrue(headers.length > 0);
    }

    public void testGetUri() {
        final String uri = this.gateway.buildGetRequest("/ws.php", new ArrayList<BasicNameValuePair>())
                .getRequestLine().getUri();
        Log.d("URI", uri);
        Assert.assertTrue(uri.contains("v=1.0.0%2Candroid%2C4.0.1"));
    }
    ...
    public void testWsParametersList() throws GatewayException {
        ...
        params.add(new BasicNameValuePair(this.getContext().getString(R.string.ws_api_key_name), this.getContext()
